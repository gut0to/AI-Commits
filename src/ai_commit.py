...
def get_diff(staged=True) -> str:
    args = ["git", "diff", "--no-color", "--unified=0"]
    if staged:
...
def main():
    ...
    if not diff.strip():
        print(" Diff vazio. Nada para descrever.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print(f" Erro chamando Gemini: {e}", file=sys.stderr)
        sys.exit(1)

    if truncated:
        message = f"{message} (truncated diff)"
    ...
    final_msg = " ".join(final_msg.splitlines()).strip()

    if not final_msg:
        print(" Mensagem vazia após edição.", file=sys.stderr)
        sys.exit(1)
